...
- Gviz R-package from Bioconductor allows making custom locus plots in R:
...
tracks in a genome browser. Let's start with at simple plot that shows genes!
...
Additionally, we can easily create chromsome ideogram and genome axis tracks.
...
chr_track <- IdeogramTrack(genome = "hg38")
...
relative height of the individual tracks.
...
By default, the genes track shows each annotated transcript. To declutter the genes
track, we can collapse all transcripts per genes into a "meta transcript".
...
the purose of a plot, it's possible to only show interactions for one gene by
...
differnt loci and in put files.
- Gviz isn't interactive and therefor not a genome browser replacement.
